...
int32_t bandHeight_FST_1 = 0;
int32_t bandHeight_FST_2 = 0;
...
void HeightSensorControl::handleMsg() {
    ...
    while (receivingRunning) {
        ...
        if (msg.code == PULSE_ADC_SAMPLE) {
            ...
            //     "ADC SAMPLE currentValue: " + std::to_string(currentValue) +
            //     " secondChance: " + std::to_string(secondChance) +
            //     " candidatesSend: " + std::to_string(candidatesSend) +
            //     " candidateValue: " + std::to_string(candidateValue), "");
...
void HeightSensorControl::processSample(
    ...
    if (!firstValue) {
        counterFirstValue++;// need to be removed
        bandHeightSum += currentValue;
        if(counterFirstValue >= 300){ // need to be removed
            if (festoNr == FESTO1){
                bandHeight_FST_1 = bandHeightSum/counterFirstValue;
                firstValue = true;
                ...
                bandHeight_FST_2 = bandHeightSum/counterFirstValue;
                firstValue = true;
    ...
    // need to redo
    } else if ((abs(currentValue - bandHeight) <= THRESHOLD)) {
        // Logger::getInstance().log(LogLevel::DEBUG, "countBandHeight++ " + std::to_string(countBandHeight) + " + 1", "HeightSensorControl");
        countBandHeight++;
        // Logger::getInstance().log(LogLevel::DEBUG, "BANDHEIGHT " + std::to_string(countBandHeight), "");
        if(countBandHeight >= 5){
            // Logger::getInstance().log(LogLevel::DEBUG, "countBandHeight(" + std::to_string(countBandHeight) + ") >= 5", "HeightSensorControl");
            handleBandHeightReached(secondChance);
            if (candidatesSend) {
                if (festoNr == FESTO1){
                    Logger::getInstance().log(LogLevel::TRACE, "sending HS1_Sampling_DONE!..." + std::to_string(currentValue), "HeightSensorControl");
        ...
        // last value very close to current value
        if (currentValue > 3600) {
            ...
            // std::cout << "HSCxONT
            if (festoNr == FESTO1){
                // Logger::getInstance().log(LogLevel::TRACE, "sending Value HS1_SAMPLE: ..." + std::to_string(currentValue), "HeightSensorControl");
