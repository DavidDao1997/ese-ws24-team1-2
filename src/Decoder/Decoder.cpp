...
void Decoder::handleMsg() {
    ...
    while (running) {
        ...
        if (recvid < 0) {
            perror("MsgReceivePulse failed!");  // TODO does this happen on
                                                // decoonstruct???
            running = false;
            return;  // TODO
        }
        ...
        if (recvid == 0) {  // Pulse received
            ...
            decode();
            std::cout
                << "Interrupt received entpacken und weitergabe an dispatcher"
                << std::endl;
        }
    }
}
...
void Decoder::decode() {
    ...
    if ((flippedValues & (uint32_t)BIT_MASK(SES_PIN)) != 0) {
        ...
        sendMsg(code, 0);  // TODO SWITCH HERE TO SECOND FESTO (instead of 0 put
                           // 1 if festo2)
    }
    // Light Barrier Front
    if ((flippedValues & (uint32_t)BIT_MASK(LBF_PIN)) != 0) {
        ...
        // TODO Add 2 Festo support eg add festo# to value instead of sending
        // just 0 and remove number -> PULSE_LBF_INTERRUPTED
